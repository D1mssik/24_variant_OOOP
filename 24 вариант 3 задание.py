...
class DataLoader:
    ...
    def load_from_file(self, filename="applications.txt"):
        try:
            with open(filename, 'r', encoding='utf-8') as file:
                for line in file:
                    if line.strip():
                        ...
                        if len(parts) >= 3:
                            app_type, description, creator_name = parts
                            creator = self.find_or_create_user(creator_name)
                            self.manager.create_application(description, app_type)
            print(f"Загружено {len(self.manager.journal.all_applications)} заявок")
    ...
    def find_or_create_user(self, name):
        for user in self.manager.users:
            if user.fullname == name:
                return user
        user_id = len(self.manager.users) + 1
        user = User(user_id, name, f"{name}@college.ru", "Отделение", "Преподаватель")
        self.manager.users.append(user)
        return user
...
class ReportGenerator:
    ...
    def generate_report(self):
        type_count = self.manager.analyze_journal()
        total = len(self.manager.journal.all_applications)
        
        report_data = []
        for app_type, count in type_count.items():
        ...
        df.to_excel('отчет_заявки.xlsx', index=False)

# ОСНОВА
def main():
    manager = ManagerApplication()
    
    data_loader = DataLoader(manager)
    report_generator = ReportGenerator(manager)
    journal_view = JournalView(manager)
    
    data_loader.load_from_file()
    
    journal_view.show_journal()
